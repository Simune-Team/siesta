...
!    - The fifth line indicates whether the grid data file SystemLabel.TASK
...
!     An extra option 'v', followed by a line with three real numbers,
...
!    In example above: graf.RHO. You should copy it from the directory
...
!
  ...
  n_opts = 0
  do
     call getopts('dz:i:s:v:X:Y:IH',opt_name,opt_arg,n_opts,iostat)
     if (iostat /= 0) exit
  ...
  origin = gf%origin
       ...
       write(6,"(a,3g20.8)") "fmin, fmax ", fmin, fmax
  ...
  subroutine manual()
    ...
    write(0,"(a)") " -v 'ux uy uz'  Tip spin direction for selection"
