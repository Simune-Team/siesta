...
! Spin capability by Alberto García (March 2019)      
c****************************************************************************
c CAVEATS:
c
...
c        unformatted
c        {x|y|z|s|q|v}      
c        [required if 'v' used above] tip_1 tip_2 tip_3
c     --- end input file ---
c
c    where:
...
c      in SIESTA)
c     - The last line indicates whether the charge (q) or a spin component
c       (x|y|z), or the total spin magnitude (s) is chosen for processing.     
c       By default, 'q' mode is chosen if no last line is specified, to
c       maintain compatibility with the older version of the program.      
c       An extra option 'v', followed by a line with three real numbers, serves to
c       simulate a spin-polarized tip. The scalar product of the "tip spin vector"
c       and the "local spin vector" is recorded at each point.
c     
...
c The program generates some informative output on standard output, and
c writes one file. In the case of 'constant-current' mode, this file's
c name is SystemLabel.*.CC.STM, and contains the X,Y,Z values of the
c isosurface (a grid of X,Y and the value of Z(X,Y) of the
c isosurface). In the case of 'constant-height' mode, the name is
c SystemLabel.*.CH.STM, and contains the values X,Y,RHO for each X,Y of
c the grid, where RHO is the charge (or spin component) computed at the
c point X,Y,Z (Z being the hight specified in the input).
c In the filenames, '*' stands for the 'spin code' (as entered in the last line).
c****************************************************************************
...
c Generate x,y,z surface (dump to output file)
       ...
       if (mode .eq. 'constant-current') then 
         call isocharge( cell, mesh, mesh, rho, fvalue, oname)
       else
