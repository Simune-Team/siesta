...
if nq==1            % if only Gamma point
  ...
  if PlotLogA == 0                         % normal scale        
      ...
      sA.EdgeColor = 'none';
      zlimit = max(max(dosA))*1.01;               
      axis([0,qpath(end),emin,emax,0,zlimit])     
      caxis([0,zlimit]);                          
  else                                     % logaritmic scale 
      sA = surf(qpath',e',max(log10(dosA),-3)');    
      zlimit = max(max(log10(dosA)))*1.05;
      sA.EdgeColor = 'none';
      axis([0,qpath(end),emin,emax,-3,zlimit])
      caxis([-3,zlimit]);
  end
  ...
  if PlotLogB == 0                         % normal scale        
      sA = surf(qpath',e',dosB');  
      sA.EdgeColor = 'none';
      zlimit = max(max(dosB))*1.01;               
      axis([0,qpath(end),emin,emax,0,zlimit])     
      caxis([0,zlimit]);                          
  else                                     % log scale 
      sA = surf(qpath',e',max(log10(dosB),-3)');    
      zlimit = max(max(log10(dosB)))*1.05;
      sA.EdgeColor = 'none';
      axis([0,qpath(end),emin,emax,-3,zlimit]) 
      caxis([-3,zlimit]);
  end
