...
program dmbs2dm
!
! Converts a blocked DM file to classic DM format
...
if (iostat /= 0) then
   print *, "File does not look like a blocked one..."
   rewind(1)
   read(1,iostat=iostat) no_u, nspin
   print *, "It looks like a classic file with Norbs, nspin: ", no_u, nspin
   print *, "Siesta can read it directly"
   stop
endif
print *, "Blocked file. Norbs, nspin, blocksize: ", no_u, nspin, blocksize
write(2) no_u, nspin
...
CONTAINS
  subroutine manual()
    write(0,'(a)') ' Usage: dmbs2dm [-h] -i BlockedFile -o ClassicFile'
    write(0,*) ! new line
