...
!!
 ...
 if (dos) then
  ...
  ! * Curves
     ...
     allocate(eig(nwfmx,nspin), fat(nwfmx,nspin))
     ...
     !Stream over file, without using too much memory
        ...
        if (dos) then
           ...
           write(fat_u,"(f14.4)") 7.77
           write(fat_u,"(3i6)")   nwfmin, min(nsp,2), nkp
        endif
        ...
        do ik=1,nkp
           ...
           do is=1,nsp
                         ...
                         ind = list_ind(ind_red)
                             ...
                             factor =  (qcos*cos(alfa)-qsin*sin(alfa)) * wk(ik)
                                ...
                                if (dos) then
                                   ...
                                   enddo
                                   fat(iw,is) = fat(iw,is) + Sover(ind)*factor/wk(ik)
                                endif
                                if (coop) then
              ...
              enddo      ! is
              if (dos) then
                 write(fat_u,"(i5,10f12.5,/,(5x,10f12.5))")  &
                        ik, ((fat(iw,is),iw=1,nwfmin),is=1,nspin)
              endif
           enddo         ! ik
