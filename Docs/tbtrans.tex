...
{\Huge \textbf{T B t r a n s \, \, \input{../version.info}}}
...
\hbox{ 
    ...
    \begin{tabular}{p{.3\linewidth}p{.6\linewidth}}
      ...
      Nick R\"ubner Papior &
      \emph{Technical University of Denmark and Institut Catal\`a
          de Nanoci\`encia i Nanotecnologia}
...
\begin{itemize}
  ...
  \item Bulk electrode density of state and transmission (directly from the electrode
  Hamiltonian)
...
The compilation is done using a \shell{Makefile} that is provided
with the code.\index{Makefile} This \shell{Makefile} will generate
the executable for any of several architectures, with a minimum of
tuning required from the user and encapsulated in a separate file
called \texttt{arch.make}.
...
and processors are not oversubscribed and are not overlapping.
For instance if using OpenMPI $\ge$1.8.4 one should run \tbtrans\
...
Several modern BLAS implementations allow the use of GEMM3M
kernels for complex linear algebra. They should provide a performance
...
\begin{fdflogicalF}{TBT.DOS!A.Elecs.All}
...
Setting the flags \fdf{TBT.DOS!Gf} and \fdf{TBT.DOS!A.Elecs.All} to
...
\begin{fdflogicalF}{TBT.T!Elecs.All}
...
\begin{fdflogicalF}{TBT.T!Out}
  ...
  \begin{align}
    \Tr[(\mathbf G - \mathbf G^\dagger)\boldsymbol \Gamma_i],
    \\
    \Tr[\mathbf G \boldsymbol \Gamma_i \mathbf G^\dagger\boldsymbol
    \Gamma_i].
  \end{align}
  The total transmission out of electrode $i$ may then be calculated
  as
  \begin{equation}
    T_i = i\Tr[(\mathbf G - \mathbf G^\dagger)\boldsymbol \Gamma_i] 
    -
    \Tr[\mathbf G \boldsymbol \Gamma_i \mathbf G^\dagger\boldsymbol \Gamma_i].
  \end{equation}
