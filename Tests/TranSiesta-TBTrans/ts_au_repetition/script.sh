...
#
 if [ -z "$TBT" ] ; then
  TBT=${ROOT_DIR}/Util/TBTrans_rep/tbtrans
  echo "==> (Compiling $TBT...)"
  # Clean in case the compiled version there is not compatible
  (cd "${ROOT_DIR}/Util/TBTrans_rep" ; make OBJDIR="$OBJDIR" clean ;
       make OBJDIR="$OBJDIR" )
  TBT="${TS_EXEC_PREFIX} ${ROOT_DIR}/Util/TBTrans_rep/tbtrans"
 fi
#
