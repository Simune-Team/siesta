...
\hrulefill
\par\vspace{3cm}
{\Large {\bf Version 3.4.0, 29 May 2014}}
\par\vspace{2cm}
\hrulefill
...
(Please note that {\sc ATOM} now depends on the SiestaXC library
...
If you are using the default compilation directory for Siesta, i.e.,
 the \texttt{Obj} directory, just type {\tt make} in the main {\sc
 ATOM} source directory ({\tt Pseudo/atom}). If not, specify the
 compilation directory's name. For example: {\tt make
 OBJDIR=Gfortran}. After a short while you will have the executable
 (called {\tt atm}) in {\tt Pseudo/atom}. The program should work
 for any atom without recompilation.
...
scripts\footnote{GNUPLOT has its issues, but it is free, and installed
...
environments, (i.e., in the presence of bonding, which always
...
(in older versions of {\sc ATOM}) are labeled starting from principal quantum number 1. 
