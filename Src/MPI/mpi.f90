...
!
!       IMPORTANT: In order to support Crays, we need a way
!       to tell MPI that when we use a data type of MPI_double_precision
!       we really mean "real*8", that is, "double" in most systems.
!       One way to do this is to use the "DAT_single" and "DAT_double" tags
!       as symbolic datatypes, and have them point to the appropriate
!       data type.
...
!       ... generic overloads for <choice> argument routines ...
        ...
        USE MPI__integer_V    ;  USE MPI__integer_S

        USE MPI__single_V      ;  USE MPI__single_S
        USE MPI__double_V      ;  USE MPI__double_S
        USE MPI__complex_V     ;  USE MPI__complex_S
        USE MPI__dcomplex_V    ;  USE MPI__dcomplex_S
      
        USE MPI__logical_V    ;  USE MPI__logical_S
        USE MPI__character_V  ;  USE MPI__character_S

!       ... these are for two different-rank <choice> arguments ...
        USE MPI__integer_VS   ;  USE MPI__integer_SV

        USE MPI__single_VS      ;  USE MPI__single_SV
        USE MPI__double_VS      ;  USE MPI__double_SV
        USE MPI__complex_VS     ;  USE MPI__complex_SV
        USE MPI__dcomplex_VS    ;  USE MPI__dcomplex_SV

        USE MPI__logical_VS   ;  USE MPI__logical_SV
        USE MPI__character_VS ;  USE MPI__character_SV
