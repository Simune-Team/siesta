...
!     indexing technology)
     ...
     $           " ** No spin-orbit potential for species ", is
      ...
      call fdf_global_get(so_strength,'SpinOrbitStrength',1.0_dp)
...
C **************************** INPUT / OUTPUT *********************************
C real*8 H(maxnh,8)        : Spin-Orbit H matrix elements
C *********************************************************************
C
      implicit none
...
C Arguments
     ...
     .     indxuo(:), isa(:)
      ...
      real(dp), intent(inout) :: H(maxnh,8), Eso
...
C *********************************************************************
      ...
      La = sqrt(li*(li+1.0d0)/2.0d0)
      Lb = sqrt(li*(li+1.0d0)-2.0d0)/2.0d0
      Lc = 0.0_dp
      if (li .ge. 3) Lc = sqrt(li*(li+1.0d0)-6.0d0)/2.0d0

      if((mi+mj).EQ.0) L(1)=real(mj)
