...
#      trick the configure scripts (for FoX).
#   2. When executables compiled with FC are flagged by the computer 
#      centers as "queuing-system-only". 
...
# Preprocessor definitions or flags, hence the two alternative
# names. The proper name must match the rules at the end of this
# file. Here we use FPPFLAGS (from 'configure' usage), but it is very common
# to use DEFS also.
...
DEFS= -WF,-DMPI -WF,-DFC_HAVE_ABORT -WF,-DCDF
#
...
#
LIBS=$(NETCDF_LIBS) $(SCALAPACK_LIBS) $(BLACS_LIBS) $(LAPACK_LIBS) $(COMP_LIBS)
...
#  - Either FPPFLAGS or DEFS must be present (see above)
