...
CONTAINS
  ...
  ! CC RC  Added for the offSpOrb
  subroutine register_in_rf_pool_KB(func,l,m,jso,func_type,indexes,gindex)
    type(rad_func), pointer :: func             ! function data
    integer, intent(in)     :: l, m, jso
    character(len=*), intent(in) :: func_type   ! mnemonic kind
    integer, intent(in)          :: indexes(:)  ! legacy indexes
    integer, intent(out)    :: gindex           ! global index
...
!--------------------------------------------------------------
  function jso(gindex) result(j)
    integer, intent(in)    :: gindex
    integer                :: j

    if (valid(gindex)) then
       j = matel_pool(gindex)%jso
    else
       call die("Invalid gindex")
    endif
  end function jso
...
! CC RC  Added for the offSpOrb
  ...
  ! CC RC  Added for the offSpOrb
  subroutine evaluate_ext_radfunc_KB(rf,ik,r,phi,grphi)

    use radial, only: rad_func, rad_get_KB
    use spher_harm, only:  rlylm
...
!        write(6,*) ' matel_registry: before calling rad_get_KB, ik=',ik
        call rad_get_KB(func,ik,rmod,phir,dphidr)
!        write(6,*) ' matel_registry: after calling rad_get_KB..'
          ilm = l*l + l + m + 1
          call rlylm( l, r, rly, grly )
          phi = phir * rly(ilm)
          do i = 1,3
  ...
  end subroutine evaluate_ext_radfunc_KB
  ! CC RC  Added for the offSpOrb
