...
!       ispin=4 => Imag(D12)
! CC RC        const = 1._dp
! CC RC        if (ispin .gt. 2) const = 2._dp
! CC RC        do io = 1,maxnh
! CC RC          Ebs = Ebs + H(io,ispin) * const * Dscf(io,ispin)
! CC RC        enddo
! CC RC      enddo
...
! CC RC             do io = 1,maxnh
! CC RC                DEharr = DEharr +  &
! CC RC                         H(io,ispin) * const * ( Dscf(io,ispin) - Dold(io,ispin) )
! CC RC             enddo
! CC RC          enddo
...
!     Compute Tr[H_0*DM_out] = Ekin + Enl with DM_out
      ...
      Ekin = 0.0_dp
      Enl  = 0.0_dp
      do ispin = 1,min(nspin,2)  ! CC RC  It is ok. It has to be spinor_dim 
        do io = 1,maxnh
