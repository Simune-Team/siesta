...
! (Not all of them can be imported from the standard mpi module)
...
! Used module procedures
  ...
  use moreParallelSubs, only: copyFile  ! Copies a file across nodes
#ifdef MPI
  ! This module should not use the instrumented routines,
  ! so it imports the vanilla module
  use mpi
#endif
...
#ifdef MPI
subroutine print_report( prog, comm )   ! Write a report of counted times 
#else
...
! Internal variables and arrays progsWriterNode
  ...
  integer :: nProgsWriterNode, progCalls, rootNode, writerNode
  logical :: found, opened, withinMPI
  integer :: myNode, nNodes, node
#ifdef MPI
...
! Make sure that parallel variables are set
   ...
   call MPI_Comm_Size( comm, nNodes, MPIerror )
#else
   myNode = 0
   nNodes = 1
#endif
...
! END DEBUG
      ...
      call MPI_Bcast( nProgsWriterNode, 1, MPI_Integer, &
                      writerNode, Comm, MPIerror )
      call MPI_Bcast( progsWriterNode, nProgsWriterNode*maxLength, &
                      MPI_Character, writerNode, Comm, MPIerror )
#endif
! DEBUG
!      if (myNode==1) then
!        print'(a,i4,2a18)', &
...
#ifdef MPI
SUBROUTINE timer_report( comm, prog, unit, file, printNow, threshold )
#else
SUBROUTINE timer_report( prog, unit, file, printNow, threshold )
#endif
...
#ifdef MPI
      call print_report( prog, comm )
    else
      call print_report( 'all', comm )
#else
