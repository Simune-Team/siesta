...
! use fox_sax !** will be imported by the psml library
...
!----------------------------------------------------------------------------
 integer                  :: iostat
 ! Handle to PSML information
 type(psml_t), pointer    :: psxml => null()
#endif
...
!pseudopotential us used as local part
 ...
 ! Upon return, psxml will point to freshly allocated memory
 ! Any previous association will be deallocated
 ! It is up to the user to deallocate psxml when no longer needed
 ! (use psml_destroy(psxml))
 call psml_reader(filpsp,psxml) 

! In the following, two grids are used:
! The "ATM3" grid is needed to call vhartree (why?) and later
! the "vlocal" routines.
! The "FHI" grid is needed for Abinit's normal business.
!
! In the spirit of a grid-agnostic PSML file, we should fill the
! data using the psml accessors.
...
!of the file default.h of the FHI98PP code
 ...
 ainc = 1.0123_dp
 ! Atomic number
 z    = psxmlAtomicNumber(psxml)
 ! Number of valence electrons
 zval = psxmlPseudoZval(psxml)
!---
...
! If there is a global logarithmic grid in the PSML file, then use it.
! Otherwise, use some default values.

if (psxmlHasGlobalLogGrid(psxml)) then
   npts  = psxmlGridNpoints(psxml) + 1
   scale = psxmlLogGridScale(psxml) 
   step  = psxmlLogGridStep(psxml) 
else
   ! Some defaults (taken from Fe)
   npts = 1123
   scale = 1.0_e-4_dp
   step =  0.0125_dp
endif

!Allocate the radial grid for the routines taken from the ATM code, 
!and define the points ---
! Note that the first element in this grid is r=0
...
!pseudoatomic orbitals)
 ...
 do il = 1, mpsang
    ...
    !  Store the semilocal components of the pseudopotential in the
    !  variable vps, required to compute the local part of the
    !  pseudopotential.  Note that il-1 = l according to the ordering
    !  of the "down" potentials
    !  For finer control, one could use the psxmlPotAngMomentum and
    !  psxmlPotentialsDown functions in the library.

    do ir = 2, npts
       vps(ir,il-1) = psxmlEvaluatePotential(psxml,"D",il,ratm(ir))
    enddo
    vps(1,il-1)      = vps(2,il-1)

    ! Note: put each l into vpspll(:,l+1) and convert to hartree
    do ir = 1, mmax
       vpspll(ir,il) = 0.5_dp * psxmlEvaluatePotential(psxml,"D",il,rad(ir))
    end do
...
! Note that this is a "neutral atom" valence charge, possibly
! rescaled, if generated by ATM3.

    !... rho(2:npts) = psxml%valence_charge%data(1:npts-1)
    do ir = 2, npts
       rho(ir) = psxmlEvaluateValenceCharge(psxml,ratm(ir))
    enddo
...
! Determine the exchange and correlation functional and parametrization
 functl = psxmlXCFunctionalType(psxml) 
 select case(psxmlXCFunctional(psxml))
   case("Ceperley-Alder")
     author = "PZ"
...
!Interpolate the radial wave functions ---
 do il = 1, mpsang
   ...
   if( use_read_wfnl ) then
      do ir = 2, npts
         ! This function is not implemented yet, but it is straightforward
         funrad(ir) = psxmlEvaluatePseudoOrbital(psxml,"D",il,ratm(ir))
      enddo
      r2        = ratm(2) / ( ratm(3) - ratm(2) )
   ...
   yp1  = ( funrad(2) - funrad(1) ) /  ( ratm(2) - ratm(1) )
   ypn  = ( funrad(npts) - funrad(npts-1) )/ ( ratm(npts) - ratm(npts-1) )
...
!    It is set up in a logarithmic grid (by historical choice, but
!    a more convenient grid might be used).

     do ir = 2, npts
        funrad(ir) = psxmlEvaluateCoreCharge(psxml,ratm(ir))
        funrad(ir) = funrad(ir)/(4.0_dp*pi*ratm(ir)**2)
     enddo
     r2        = ratm(2) / ( ratm(3) - ratm(2) )
     funrad(1) = funrad(2) - r2 * ( funrad(3) - funrad(2) )
...
! At this point, the ATM grid is no longer needed.
