...
!
!
! Helper routines taken from xmlf90 which are
! missing in FoX

public :: get_value_by_key, get_value_by_index
...
!
CONTAINS
      subroutine get_value_by_key(dict,attr,value,status)
        use FoX_common, only: fox_get_value=>getValue, dictionary_t
        ...
        status = 0
        value = fox_get_value(dict,attr)
        if (value == "") status = 1
      end subroutine get_value_by_key

      subroutine get_value_by_index(dict,i,value,status)
        use FoX_common, only: fox_get_value=>getValue, dictionary_t
        type(dictionary_t), intent(in) :: dict
        integer, intent(in)            :: i
        character(len=*), intent(out)  :: value
        ...
        status = 0
        value = fox_get_value(dict,i)
        if (value == "") status = 1
      end subroutine get_value_by_index

      subroutine get_key(dict,i,key,status)
        use FoX_common, only: fox_get_key=>getQname, dictionary_t
        type(dictionary_t), intent(in) :: dict
        integer, intent(in)            :: i
