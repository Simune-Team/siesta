...
select case(name)
      ...
      case ("psml")
         ...
         if (value(1:3) /= PSML_TARGET_VERSION) then
            call die("Can only work with PSML files with version " &
                      // PSML_TARGET_VERSION)
         endif

         ! Initialize counters

         pseudo%semilocal%npots = 0
         pseudo%psoperator%nprojs = 0
      ...
      case ("libxc-info")
         ...
         allocate(xp%libxc_weight(n_funct))
...
!            cp => pseudo%config_core
      ...
      case ("slps")
         ...
         ! Possibly extend size of arrays
         ...
         slp%set(i) = setcode_of_string(value)
      ...
      case ("proj")
         ...
         pop%set(i) = setcode_of_string(value)
      ...
      case ("data")
         ...
         if (.not. associated(rp%grid)) then
            ...
            if (in_psoperator .and. associated(pop%grid)) then
                  rp%grid => pop%grid
            else ! try global
               if (associated(pseudo%global_grid)) then
      ...
      case ("radfunc")
         ...
         if (     in_slps .or. in_coreCharge .or. in_valenceCharge &
             .or. in_pswf .or. in_proj .or. in_local_potential) then           
            in_radfunc = .true.
         else
      ...
      case ("local-potential")
         ...
         call get_value(attributes,"type",pop%vlocal_type,status)
