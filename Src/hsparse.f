...
C                   Check maxnkb - if too small then
C                   increase array sizes
...
C     Find optimum value for nlhmax
      ...
      nlhmax = nlh
C     (re)allocate listh, but do not shrink, as it might
C     be used for DM extrapolation
C     using information from different geometries.
...
C           through a KB projector
            do jnat = 1,nna
              ...
              do jo = lasto(ja-1)+1,lasto(ja)
                if (conect(jo)) then
                  if (.not. gamma) then
C                   If already connected and using supercell, 
C                   the latter might not be big enough...
C                   We warn the user and keep the first instance
C                   of xij (same behavior as the old xijorb, as
C                   earlier jnats are closer)
                    if (.not.warn1) then
                      if (Node.eq.0) then
                        write(6,'(/,a,2i6,a,/)')
