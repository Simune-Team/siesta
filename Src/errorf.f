      ...
      ! Note that all relevant constants have been chosen for IEEE "dp",
      ! which is selected by this statement
      integer, parameter :: dp = selected_real_kind(10,100)

      ! If a platform has a different idea of "double precision", it
      ! will be flagged by the compiler as an incompatibility between
      ! the interfaces of calerf (untouched below) and derf/derfc (forced to dp)
      ...
      CONTAINS
!*****************************************************************
! subroutine calerf and functions derf and derfc
!
! Error function (and complementary) calculated
! By W. J. Cody, January 8, 1985, distributed by Netlib (public domain)
!*****************************************************************
...
C     DOUBLE PRECISION FUNCTION DERFCX(X)
C------------------------------------------------------------------
C
...
C------------------------------------------------------------------
C     INTEGER JINT
C     DOUBLE PRECISION X, RESULT
C------------------------------------------------------------------
C     JINT = 2
C     CALL CALERF(X,RESULT,JINT)
C     DERFCX = RESULT
C     RETURN
