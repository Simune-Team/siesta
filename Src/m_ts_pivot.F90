...
contains
  ...
  subroutine ts_pivot( dit, sp, &
    ...
    if ( .not. pvt_orb ) then
       ...
       ! per atom, no need to transform to atomic sparsity pattern,
       ! they are equivalent.
       pvt_orb = sum(lasto(1:na_u) - lasto(0:na_u-1)) == na_u
    end if
    if ( pvt_orb ) then
       str_tmp = 'orb'
    ...
    if ( str_contain(pvt_str,'none') ) then
       ...
       fan2 = 0
       do i = 1 , N_Elec
          if ( str_contain(pvt_str,Elecs(i)%name) ) then
